...
internal class MatchScheduler
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // tournamentMatches contains all matches of the tournament
        // (including matches of other rounds that were already saved).
        // This method saves only new or modified matches.
        OnBeforeSave?.Invoke(this, tournamentMatches);
        await _appDb.GenericRepository.SaveEntitiesAsync(tournamentMatches, true, false, cancellationToken);
        OnAfterSave?.Invoke(this, tournamentMatches);
        
        await _availableMatchDates.ClearAsync(MatchDateClearOption.OnlyAutoGenerated, cancellationToken);
    }

    /// <summary>
    /// This event is raised before the matches are saved to the database.
    /// It can be used to identify the modified or new matches that will be saved.
    /// </summary>
    internal EventHandler<EntityCollection<MatchEntity>>? OnBeforeSave;

    /// <summary>
    /// This event is raised after the matches are saved to the database.
    /// The entity collection was re-fetched after save.
    /// </summary>
    internal EventHandler<EntityCollection<MatchEntity>>? OnAfterSave;
    ...
    /// <param name="tournamentMatches">Contains the scheduled matches.</param>
